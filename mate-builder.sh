...
GITHUB_URL="https://github.com/mate-desktop"
...
action_gitup() {
	if [ ! -d "${1}" ]; then
		...
		git clone --recurse-submodules ${GITHUB_URL}/${2}${1}.git && git submodule init && git submodule update --remote --recursive || exit 1
	fi
	if [ ! -d "${1}" ]; then
